...
public abstract class JsonNode extends JsonSerializable.Base implements TreeNode, Iterable<JsonNode> {
    ...
    public final boolean isValueNode() {
        int i = fun1.$SwitchMap$com$fasterxml$jackson$databind$node$JsonNodeType[getNodeType().ordinal()];
...
JsonNode not of type ObjectNode (but " + getClass().getName() + "), can not call with() on it");
    }
...
//throw new UnsupportedOperationException(
JsonNode not of type ObjectNode (but " + getClass().getName() + "), can not call withArray() on it");
    }

...
public class NameValueBlockReader {
    ...
    static int Fun$022(NameValueBlockReader nameValueBlockReader, long j) {
    ...
    public NameValueBlockReader(BufferedSource bufferedSource) {
        InflaterSource inflaterSource = new InflaterSource(new ForwardingSource(bufferedSource) {
            @Override
            public long read(Buffer buffer, long j) throws IOException {
                if (compressedLimit == 0) {
                    return -1L;
                }
                long read = super.read(buffer, Math.min(j, compressedLimit));
                if (read == -1) {
                    return -1L;
                }
                NameValueBlockReader.Fun$022(NameValueBlockReader.this, read);
                return read;
            }
        }, new Inflater() {
            @Override

...
public final class DiskLruCache implements Closeable {
    ...
    public synchronized Iterator<Snapshot> snapshots() throws IOException {
        ...
        return new Iterator<Snapshot>() {
            ...
            {
                this.delegate = new ArrayList(lruEntries.values()).iterator();
            }
    ...
    public final class Editor {
        ...
        public Source newSource(int i) throws IOException {
            synchronized (DiskLruCache.this) {
                ...
                if (this.entry.readable) {
                    try {
                        return fileSystem.source(this.entry.cleanFiles[i]);
        ...
        public Sink newSink(int i) throws IOException {
            ...
            synchronized (DiskLruCache.this) {
                ...
                try {
                    faultHidingSink = new FaultHidingSink(fileSystem.sink(this.entry.dirtyFiles[i])) {
                        @Override
                        protected void onException(IOException iOException) {
                            synchronized (DiskLruCache.this) {
    ...
    public final class Entry {
        ...
        private Entry(String str) {
            ...
            for (int i = 0; i < valueCount; i++) {
                sb.append(i);
                this.cleanFiles[i] = new File(directory, sb.toString());
                sb.append(DefaultDiskStorage.FileType.TEMP);
                this.dirtyFiles[i] = new File(directory, sb.toString());
                sb.setLength(length);
            }
        }
        ...
        Snapshot snapshot() {
            ...
            if (Thread.holdsLock(DiskLruCache.this)) {
                Source[] sourceArr = new Source[valueCount];
                long[] jArr = (long[]) this.lengths.clone();
                for (int i = 0; i < valueCount; i++) {
                    try {
                        sourceArr[i] = fileSystem.source(this.cleanFiles[i]);
                    } catch (FileNotFoundException unused) {
                        for (int i2 = 0; i2 < valueCount && (source = sourceArr[i2]) != null; i2++) {
                            Util.closeQuietly(source);
                        }
                        return null;
                    }

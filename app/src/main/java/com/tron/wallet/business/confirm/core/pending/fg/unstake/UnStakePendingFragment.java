...
public class UnStakePendingFragment extends BaseFragment<ConfirmPendingPresenter, ConfirmPendingModel> implements ConfirmPendingContract.View {
    private static final int containerViewId = 100385;
    protected BaseParam baseParam;
    protected int failedCount;
    protected State state;

    public static UnStakePendingFragment getInstance(State state, BaseParam baseParam, byte[] bArr, int i) {
        UnStakePendingFragment unStakePendingFragment = new UnStakePendingFragment();
        ...
        unStakePendingFragment.setArguments(bundle);
        return unStakePendingFragment;
    }

    @Override
    public void onSuccess(TransactionInfoBean transactionInfoBean, BaseParam baseParam) {
        replaceFragment(UnStakeSuccessFragment.getInstance(baseParam));
    }

    @Override
    public void onFail(BaseParam baseParam, int i, String str) {
        replaceFragment(UnStakeFailureFragment.getInstance(baseParam, i));
    }

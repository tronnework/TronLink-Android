...
public class StakeSuccessPresenter extends StakeSuccessContract.Presenter {
    ...
    public void initVoteData() {
        ...
        Observable.zip(((StakeSuccessContract.Model) this.mModel).getAccount(null, this.controllerAddress), ((StakeSuccessContract.Model) this.mModel).getAccountResourceMessage(null, this.controllerAddress), ((StakeSuccessContract.Model) this.mModel).requestWitnessList(this.controllerAddress, 0, 6, 3, 1), new Function3() {
            @Override
            ...
            public void onResponse(String str, Pair<Long, ArrayList<VoteWitnessBean>> pair) {
                ((StakeSuccessContract.View) mView).dismissLoadingDialog();
                if (pair.first != null && pair.second != null) {
                    long longValue = ((Long) pair.first).longValue();
                    ArrayList arrayList = (ArrayList) pair.second;
                    ((StakeSuccessContract.View) mView).updateVoteApr(arrayList, longValue);
                    ((StakeSuccessContract.View) mView).showVotes();
                    adapter = new StakeSuccessVoteAdapter(((StakeSuccessContract.View) mView).getIContext(), arrayList);
                    ((StakeSuccessContract.View) mView).getRecyclerView().setLayoutManager(new WrapContentLinearLayoutManager(((StakeSuccessContract.View) mView).getIContext(), 1, false));
                    ((StakeSuccessContract.View) mView).getRecyclerView().setAdapter(adapter);
                }
                isLoading = false;
            }
            ...
            public void onFailure(String str, String str2) {
                ((StakeSuccessContract.View) mView).showLoadFailed();
            }
    ...
    public void VoteToProfit() {
        ...
        if (selectedWallet != null && selectedWallet.isWatchOnly() && !SpAPI.THIS.getCurrentChain().isMainChain) {
            ...
            ((StakeSuccessContract.View) this.mView).runOnThreeThread(new OnBackground() {
                @Override
    ...
    public void lambda$VoteToProfit$6(final Wallet wallet, final HashMap hashMap, final HashMap hashMap2, final int i, final String str, final DataStatHelper.StatAction statAction) {
        ((StakeSuccessContract.Model) this.mModel).getAccount(null, this.controllerAddress).subscribe(new Consumer<Protocol.Account>() {
            @Override
                ...
                if (account != null) {
                    try {
                        if (!WalletUtils.checkHaveOwnerPermissions(wallet.getAddress(), account.getOwnerPermission())) {
                            ((StakeSuccessContract.View) mView).runOnUIThread(new OnMainThread() {
                                @Override
                            ...
                            try {
                                ...
                                if (transactionExtention != null && transactionExtention.hasResult() && !"".equals(createVoteWitnessTransaction2.getTransaction().toString())) {
                                    ...
                                    if (transaction != null) {
                                        ...
                                        ConfirmTransactionNewActivity.startActivity(context, ParamBuildUtils.getVoteTransactionParamBuilder(hashMap2, hashMap, true, 1, transaction, i, controllerAddress, 0L, str, true, statAction, ((StakeSuccessContract.View) mView).getFormatApr()), wallet2.getCreateType() == 7);
                                    } else {
                                        bundle.putString("value", "0");
                                        FirebaseAnalytics.getInstance(context).logEvent("Click_vote_button", bundle);
                                        ((StakeSuccessContract.View) mView).toast(((StakeSuccessContract.View) mView).getIContext().getString(R.string.time_out));
                                    }
                                } else {
                                    bundle.putString("value", "0");
                                    FirebaseAnalytics.getInstance(context).logEvent("Click_vote_button", bundle);
                                    ...
                                    if (transactionExtention2 != null && transactionExtention2.hasResult() && 2 == createVoteWitnessTransaction2.getResult().getCode().getNumber()) {
                                        ((StakeSuccessContract.View) mView).toast(((StakeSuccessContract.View) mView).getIContext().getString(R.string.CONTRACT_VALIDATE_ERROR));
                                    } else {
                                        ((StakeSuccessContract.View) mView).toast(((StakeSuccessContract.View) mView).getIContext().getString(R.string.time_out));
                                    }
                                }
            ...
            public void accept(final Protocol.Account account) throws Exception {
                if (account != null) {
                    ...
                    ((StakeSuccessContract.View) mView).runOnUIThread(new OnMainThread() {
                        ...
                        public void doInUIThread() {
                            ((StakeSuccessContract.View) mView).dismissLoadingDialog();
                            VoteSelectSRActivity.start(((StakeSuccessContract.View) mView).getIContext(), account, false, controllerAddress, null, permissionData, 5, votingTps, true, DataStatHelper.StatAction.Vote);
                        }
                    });
                }
            }
    ...
    public void showMultiSignDialog() {
        ConfirmCustomPopupView.getBuilder(((StakeSuccessContract.View) this.mView).getIContext()).setTitle(((StakeSuccessContract.View) this.mView).getIContext().getString(R.string.vote_no_owner_permission)).setTitleSize(16).setPopupCallback(new SimpleCallback() {
            @Override
            public void onDismiss(BasePopupView basePopupView) {
            }
        }).setConfirmText(((StakeSuccessContract.View) this.mView).getIContext().getString(R.string.multisig)).setCancleListener(new View.OnClickListener() {
            @Override

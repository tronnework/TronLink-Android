...
public class DappTriggerSmartContractDetailFragment extends BaseFragment<DappTransferTransactionDetailPresenter, EmptyModel> implements DappTransferTransactionDetailContract.View, BaseConfirmFragment.AccountCallback {
    ...
    public static DappTriggerSmartContractDetailFragment getInstance(BaseParam baseParam) {
        DappTriggerSmartContractDetailFragment dappTriggerSmartContractDetailFragment = new DappTriggerSmartContractDetailFragment();
        Bundle bundle = new Bundle();
        bundle.putParcelable(ArgumentKeys.KEY_PARAM, baseParam);
        dappTriggerSmartContractDetailFragment.setArguments(bundle);
        return dappTriggerSmartContractDetailFragment;
    }
    ...
    private void setChildrenVisibility() {
        ...
        if (triggerSmartContract.getTokenId() > 0 && triggerSmartContract.getCallTokenValue() > 0) {
            TokenBean queryToken = AssetsManager.getInstance().queryToken(WalletUtils.getSelectedWallet().getAddress(), String.valueOf(triggerSmartContract.getTokenId()));
            if (queryToken != null) {
                this.numberFormat.setMaximumFractionDigits(queryToken.getPrecision());
                StringBuilder sb = new StringBuilder(Marker.ANY_NON_NULL_MARKER);
                sb.append(this.numberFormat.format(triggerSmartContract.getCallTokenValue() / ((long) Math.pow(10.0d, queryToken.getPrecision()))));
                if (triggerSmartContract.getTokenId() == 1002000) {
                    str2 = " BTT";
                } else if (queryToken.getShortName() != null) {
                    str2 = queryToken.getShortName();
                }
                sb.append(str2);
                str2 = sb.toString();
            } else {
                this.numberFormat.setMaximumFractionDigits(6);
                str2 = Marker.ANY_NON_NULL_MARKER + this.numberFormat.format(triggerSmartContract.getCallTokenValue());
            }
        }
        ...
        NoDoubleClickListener2 noDoubleClickListener2 = new NoDoubleClickListener2() {
            ...
            protected void onNoDoubleClick(View view) {
                ...
                if (id == R.id.rl_contract_address) {
                    ...
                    DappTriggerSmartContractDetailFragment dappTriggerSmartContractDetailFragment = DappTriggerSmartContractDetailFragment.this;
                    CommonWebActivityV3.start((Context) activity, dappTriggerSmartContractDetailFragment.getTrc10Url(dappTriggerSmartContractDetailFragment.param.getAssetName()), mContext.getResources().getString(R.string.tronscan), -2, true);
                    try {
                        if (!Protocol.Transaction.parseFrom(param.getTransactionBean().getBytes().get(0)).getRawData().getContract(0).getType().equals(Protocol.Transaction.Contract.ContractType.TriggerSmartContract)) {
                            i = 1;
                        }

...
public class OutboundFlowController {
    ...
    public final class OutboundFlowState {
        ...
        int writableWindow() {
            return Math.min(this.window, connectionState.window());
        }
        ...
        void write(Buffer buffer, int i, boolean z) {
            do {
                ...
                int i2 = -min;
                connectionState.incrementStreamWindow(i2);
                incrementStreamWindow(i2);
                try {
                    frameWriter.data(buffer.size() == ((long) min) && z, this.streamId, buffer, min);
